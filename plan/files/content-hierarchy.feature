Feature: Content Hierarchy Browsing
  ...
  Scenario: List product types for Netherlands
    ...
    And match response.data[0].productCount > 0
  ...
  Scenario: Domains show locked status for free user
    ...
    When method GET
    Then status 200
    # First 2 domains are free preview
    And match response.data[0].isFreePreview == true
    And match response.data[0].isLocked == false
    And match response.data[1].isFreePreview == true
    And match response.data[1].isLocked == false
  ...
  Scenario: List road signs for Netherlands
    ...
    When method GET
    Then status 200
    And match each response.data contains { code: '#string', name: '#string', signCategory: '#string', imageUrl: '#string' }
  ...
  Scenario: Invalid country code returns empty list
  ...
  Scenario: Invalid product type code returns empty list
