Feature: Progress, Readiness, Streaks, Achievements, and Study Plan
  ...
  Scenario: Get readiness score for user with progress
    ...
    And match response.data.readinessScore <= 100
    ...
    And match response.data.domainStrengths == '#[? _.length > 0]'
    And match each response.data.domainStrengths contains { domainCode: '#string', strength: '#string', accuracyPercent: '#number', coveragePercent: '#number' }
  ...
  Scenario: Readiness trend returns daily snapshots
    ...
    And match each response.data contains { date: '#string', readinessScore: '#number' }
  ...
  Scenario: Get domain progress breakdown
    ...
    And match each response.data contains { domainCode: '#string', totalQuestions: '#number', attemptedCount: '#number', accuracyPercent: '#number', strength: '#string' }
  ...
  Scenario: Get mastery level distribution
    ...
    Then status 200
    And match response.data.totalQuestions == '#number'
    And match response.data.new == '#number'
    And match response.data.learning == '#number'
    And match response.data.familiar == '#number'
    And match response.data.mastered == '#number'
    # Counts should sum to total
    And assert response.data.new + response.data.learning + response.data.familiar + response.data.mastered == response.data.totalQuestions
  ...
  Scenario: Fresh user has all questions as NEW
    ...
    When method GET
    Then status 200
    And match response.data.new == response.data.totalQuestions
    And match response.data.learning == 0
    And match response.data.familiar == 0
    And match response.data.mastered == 0
  ...
  Scenario: First question triggers first_question achievement
    ...
    * def questionId = response.data.currentQuestion.strapiQuestionId
    ...
    When method POST
    Then status 200
    And match response.data.newAchievements == '#[? _.length > 0]'
  ...
  Scenario: Generate study plan with exam date
    ...
    And headers paidHeaders
    And request { productCode: 'auto-b', examDate: '2026-04-20', dailyQuestionTarget: 20 }
    When method POST
    Then status 200
    And match response.data.planId == '#uuid'
    And match response.data.productCode == 'auto-b'
    And match response.data.examDate == '2026-04-20'
    ...
    And match response.data.totalDays >= 7
    And match response.data.focusDomains == '#array'
    And match response.data.days == '#[? _.length > 0]'
  ...
  Scenario: Study plan allocates more days to weak domains
    Given path '/api/study-plan'
    And headers paidHeaders
    And request { productCode: 'auto-b', examDate: '2026-05-01', dailyQuestionTarget: 20 }
    When method POST
    Then status 200
    And match response.data.focusDomains == '#[? _.length > 0]'
