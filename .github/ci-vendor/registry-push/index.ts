...
// PassTheo: USERNAME_REGISTRY defaults to "ci-push" so callers only need
// to pipe the JWT on stdin. The Worker's auth path strictly requires the
// Basic scheme (serverless-registry src/auth.ts:38) and treats the
// Authorization password as the JWT — so we keep the upstream Basic header
// shape with the JWT as the password.
const username = process.env["USERNAME_REGISTRY"] ?? "ci-push";
...
if (process.stdin.isTTY) {
  console.error(
    "You need to pass the JWT with a pipe operator \n\n\t'echo <JWT> | bun run index.ts <image>'\n",
  );
} else {
...
if (!password) {
  console.error("JWT not provided on stdin; push won't be able to authenticate with registry");
...
for (const layer of manifest.Layers) {
  tasks.push(
    pool(async () => {
      ...
      // PassTheo: upstream lacked `force: true`, which makes Bun/newer Node
      // throw ENOENT on the first run when the .cache dir is empty. The
      // intent is "delete if present", so add force.
